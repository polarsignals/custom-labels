...
use otel_thread_ctx::otel_process_ctx;
...
fn main() {
        ...
        resource: Some(Resource {
            attributes: vec![
                KeyValue {
                    ...
                    value: string_val("spin-example"),
                    key_ref: 0,
                },
                KeyValue {
                    key: "service.version".into(),
                    value: string_val("0.5.0"),
                    key_ref: 0,
                },
            ],
            ...
            KeyValue {
                ...
                value: Some(AnyValue {
                    value: Some(any_value::Value::ArrayValue(ArrayValue {
                        values: vec![AnyValue {
                            value: Some(any_value::Value::StringValue("label".into())),
                        }],
                    })),
                }),
                key_ref: 0,
    ...
    let span_id = [2u8; 8];
    // Initial attach
    ThreadContext::new(trace_id, span_id, &[(0, &rand_str())]).attach();

    loop {
        // In-place update (no allocation)
        ThreadContext::update(trace_id, span_id, &[(0, &rand_str())]);
